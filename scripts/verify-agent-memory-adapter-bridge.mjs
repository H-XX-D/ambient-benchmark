#!/usr/bin/env node
// Smoke for adapters/agent-memory-sqlite-adapter.mjs.
//
// Verifies AMBIENT /write, /query, /collection, reset isolation, and a
// baiXfeng/agent-memory-compatible storage-dir memory.db schema.
...
async function main() {
  const roots = await mkdtemp(join(tmpdir(), "ambient-agent-memory-roots-"));
  ...
  const bridge = spawn(
    process.execPath,
    [
      "adapters/agent-memory-sqlite-adapter.mjs",
      "--root",
      roots,
      "--port",
      String(port),
  ...
  try {
    ...
    if (name.name !== "agent-memory-sqlite") throw new Error(`unexpected name ${JSON.stringify(name)}`);

    await post(base, "/reset", { store: "custom" });
    const write = await post(base, "/write", {
      store: "custom",
      fact: "agent-memory stores local memories in storage-dir memory.db with FTS5 trigram search",
      source: "smoke",
    });
    if (!write.accepted || !existsSync(write.db)) {
      throw new Error(`agent-memory memory.db was not created: ${JSON.stringify(write)}`);
    }

    const hit = await post(base, "/query", { store: "custom", question: "storage-dir memory.db FTS5", top_k: 3 });
    if (!hit.support.some((s) => s.includes("agent-memory"))) {
...
row = conn.execute("SELECT title, summary, content, keywords, category FROM memories LIMIT 1").fetchone()
fts = conn.execute("SELECT m.content FROM memories_fts f JOIN memories m ON m.id = f.rowid WHERE memories_fts MATCH ? LIMIT 1", ('"FTS5"',)).fetchone()
print(json.dumps({"tables": sorted(tables), "row": row, "fts": fts}))
      `,
      [write.db],
    );
    const schema = JSON.parse(stdout);
    for (const table of ["memories", "memories_fts"]) {
      if (!schema.tables.includes(table)) throw new Error(`missing compatible table ${table}: ${stdout}`);
    }
    if (!schema.row || !String(schema.row[2]).includes("agent-memory")) {
      throw new Error(`memory row probe failed: ${stdout}`);
    }
    if (!schema.fts || !String(schema.fts[0]).includes("agent-memory")) {
      throw new Error(`FTS probe failed: ${stdout}`);
    }

    await post(base, "/reset", { store: "custom" });
    const miss = await post(base, "/query", { store: "custom", question: "storage-dir memory.db FTS5", top_k: 3 });
    if (miss.support.length) {
      throw new Error(`reset did not isolate agent-memory DB: ${JSON.stringify(miss)}`);
    }
    console.log("agent-memory bridge smoke: write/query/reset isolated storage-dir memory.db verified");
  } finally {
    bridge.kill("SIGTERM");
